...
var instantiateChaincode = async function(peers, channelName, chaincodeName, chaincodeVersion, functionName, chaincodeType, args, username, org_name) {
	...
	try {
		...
		if (chaincodeVersion != "v0"){
			var results = await channel.sendUpgradeProposal(request, 600000); //instantiate takes much longer
		} else {
			var results = await channel.sendInstantiateProposal(request, 600000); //instantiate takes much longer
		}
		...
		for (var i in proposalResponses) {
			...
			if (proposalResponses && proposalResponses[i].response &&
				...
				logger.error('instantiate proposal was bad',proposalResponses);
			}

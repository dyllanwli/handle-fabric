...
for (let key in ORGS) {
	if (key.indexOf('org') === 0) {
		...
		let caUrl = ORGS[key].ca.requests;
...
function newRemotes(names, forPeers, userOrg) {
	...
	for (let idx in names) {
		...
		if (ORGS[userOrg][peerName]) {
			// found a peer matching the name
			let data = fs.readFileSync(path.join(__dirname, ORGS[userOrg][peerName]['tls_cacerts']));
			let grpcOpts = {
				pem: Buffer.from(data).toString(),
				'ssl-target-name-override': ORGS[userOrg][peerName]['server-hostname']
			};

			if (forPeers) {
				targets.push(client.newPeer(ORGS[userOrg][peerName].requests, grpcOpts));
			} else {
				let eh = client.newEventHub();
				eh.setPeerAddr(ORGS[userOrg][peerName].events, grpcOpts);
				targets.push(eh);
			}
		}
	}
